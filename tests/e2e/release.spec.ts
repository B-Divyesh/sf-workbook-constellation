...
test('keeps the shipped release available when a newer-release check fails', async ({ page }) => {
  ...
  await page.goto('/');
  await expect(page.getByRole('link', { name: 'Download for Linux (external)' })).toHaveAttribute('href', /v0\.1\.9/);
  await page.getByRole('button', { name: 'Check for a newer release' }).click();

  await expect(page.getByText('GitHub is unavailable. Showing v0.1.9.')).toBeVisible();
  await expect(page.getByRole('link', { name: /See all release files/ })).toHaveAttribute('href', /releases\/tag\/v0\.1\.9$/);
