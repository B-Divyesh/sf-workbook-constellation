...
function landing() {
  ...
  return shell(`<main id="main">
    ...
    <section class="price" aria-labelledby="price-title"><div><p class="eyebrow">Constellation Plus</p><h2 id="price-title">Audit larger workbooks for $19 once</h2><p>One license accepts workbooks above 8 sheets and adds JSON evidence export. HTML handoff reports stay free.</p></div><a class="primary" href="${checkoutUrl}">Buy a $19 license <span class="sr-only">(external)</span></a><form id="license-form"><label for="license">Have a license?</label><div><input id="license" name="license" autocomplete="off" required><button type="submit">Verify license</button></div><p id="license-status" role="status"></p></form></section>
  </main>`);
}
