...
import { appVersion, buildCommit } from './version';
...
function shell(content: string) {
  ...
  ${content}
  <footer><p><strong>Workbook Constellation</strong><br>Map workbook formulas before you change a cell.</p><nav aria-label="Footer navigation"><a href="/privacy" data-link>Privacy</a><a href="/terms" data-link>Terms</a><a href="https://sociobot.in" rel="noreferrer">Built by Param Factory <span class="sr-only">(external)</span></a></nav><p>Version ${appVersion} · Build ${buildCommit.slice(0, 12)} · Original generated artwork</p></footer>`;
}
...
function landing() {
  ...
  return shell(`<main id="main">
    <section class="hero">
      ...
      <div class="hero-actions"><button class="primary" data-action="sample">Try it with sample data</button><span>See a completed map of formula paths between sheets.</span></div>
    ...
    <section class="workspace-shell" aria-labelledby="workspace-title"><div class="section-kicker">Open a workbook</div><h2 id="workspace-title">Open a workbook in read-only mode</h2><p>Choose an XLSX or XLSM file. The report reads saved formulas only. It never runs macros or opens linked files.</p>
      <div class="drop-zone" id="drop-zone"><input id="file" type="file" accept=".xlsx,.xlsm" aria-describedby="file-help"><label class="primary" for="file">Choose an XLSX or XLSM file</label><span id="file-help">or drop one here · 50 MB maximum</span></div><p id="file-status" class="status" role="status"></p>
    </section>
    <section id="preview" class="preview-band" aria-labelledby="preview-title"><div><p class="section-kicker">Live preview</p><h2 id="preview-title">Preview formula paths between sheets</h2><p>Select a sheet or a path. The evidence panel lists the exact cells behind it.</p></div><div class="mini-map" aria-hidden="true"><span>Orders</span><i></i><span>Revenue</span><i></i><span>Forecast</span><i></i><span>Dashboard</span></div></section>
    <section class="walkthrough" aria-labelledby="walkthrough-title"><p class="section-kicker">Desktop walkthrough</p><h2 id="walkthrough-title">Desktop workbook walkthrough</h2><div class="walkthrough-grid"><figure><img src="/art/walkthrough-open-b6a40aa5.webp" width="960" height="600" loading="lazy" alt="The sample workbook open in the desktop map."><figcaption><strong>1. Open a workbook.</strong> The map lists sheets, formulas, paths, and warnings.</figcaption></figure><figure><img src="/art/walkthrough-path-547dfed0.webp" width="960" height="600" loading="lazy" alt="A selected path with source and destination cells shown."><figcaption><strong>2. Inspect a path.</strong> Select a path to see its source cells and saved formula.</figcaption></figure><figure><img src="/art/walkthrough-report-a187e4c6.webp" width="960" height="600" loading="lazy" alt="The exported HTML report open in a browser."><figcaption><strong>3. Save the report.</strong> Open the HTML file later without Workbook Constellation.</figcaption></figure></div></section>
    <section id="how" class="steps"><p class="section-kicker">How it works</p><h2>Map and export a workbook in three steps</h2><ol><li><b>1</b><h3>Open the workbook</h3><p>Choose an XLSX or XLSM file. Macro code is never run.</p></li><li><b>2</b><h3>Inspect the paths</h3><p>Trace formula paths between sheets. Review external workbook links, circular references, and formulas the app cannot trace.</p></li><li><b>3</b><h3>Export the report</h3><p>Save an HTML report that opens without this app.</p></li></ol></section>
    ...
    <section class="downloads" aria-labelledby="download-title"><div><p class="section-kicker">Desktop app</p><h2 id="download-title">Keep workbook audits on your computer</h2><p>The desktop build is unsigned. Compare the download’s SHA-256 checksum with the release page before opening it.</p></div><div id="download-action" class="download-action" aria-live="polite"><span>Loading release details…</span></div></section>
    <section class="price" aria-labelledby="price-title"><div><p class="eyebrow">Constellation Plus</p><h2 id="price-title">Audit larger workbooks for $19 once</h2><p>One license accepts workbooks above 8 sheets and adds JSON evidence export. HTML reports stay free.</p></div><a class="primary" href="${checkoutUrl}">Buy a $19 license <span class="sr-only">(external)</span></a><form id="license-form"><label for="license">License token</label><div><input id="license" name="license" autocomplete="off" required><button type="submit">Verify license</button></div><p id="license-status" role="status"></p></form></section>
  </main>`);
}
...
function notFound() {
  ...
  return shell(`<main id="main" class="not-found"><div aria-hidden="true">404</div><h1 tabindex="-1">Page not found</h1><p>The address does not match a page in Workbook Constellation.</p><a class="primary" href="/" data-link>Return to Workbook Constellation</a></main>`);
}
