# Landing copy audit

Audited 2026-08-28. Counts treat hyphenated terms and numerals as one word. Headings, labels, buttons, and fragments are included for clarity. No line exceeds 22 words. No banned marketing word appears.

| Copy | Words | Result |
|---|---:|---|
| Read-only workbook map | 3 | pass |
| Map workbook formulas before you edit | 6 | pass |
| For people inheriting complex workbooks who need to trace cross-tab sources before making changes. | 14 | pass |
| Try it with sample data | 5 | pass |
| See a finished dependency map. | 5 | pass |
| Files stay on this device | 5 | pass |
| Works without an account | 4 | pass |
| Free for workbooks up to 8 sheets | 7 | pass |
| Open a workbook in read-only mode | 6 | pass |
| Choose an XLSX file. | 4 | pass |
| The report uses formula records only. | 6 | pass |
| It never runs macros or opens linked files. | 8 | pass |
| or drop one here · 50 MB maximum | 8 | pass |
| Follow each formula to its source | 6 | pass |
| Select a sheet or a path. | 6 | pass |
| The evidence panel lists the exact cells behind it. | 9 | pass |
| Three steps from file to handoff | 6 | pass |
| Choose an XLSX or XLSM file. | 6 | pass |
| Macro code is never run. | 5 | pass |
| Trace tab links and review external, circular, or opaque formulas. | 10 | pass |
| Save a static HTML report for the workbook’s next owner. | 10 | pass |
| Structural proof, not calculated answers | 5 | pass |
| Workbook Constellation does not edit cells, calculate formulas, run macros, or open external links. | 14 | pass |
| Encrypted files and formulas stored only inside unsupported add-ins cannot be read. | 12 | pass |
| Keep workbook audits on your computer | 6 | pass |
| The desktop build is unsigned. | 5 | pass |
| Check its published SHA256 before opening it. | 7 | pass |
| Audit larger workbooks for $19 once | 6 | pass |
| One license adds unlimited sheets and JSON evidence export. | 9 | pass |
| HTML handoff reports stay free. | 5 | pass |

## Terminology

| Concept | One term |
|---|---|
| An uploaded spreadsheet file | workbook |
| A workbook tab | sheet |
| A formula connection between sheets | path |
| Exact source and destination cells | evidence |
| Portable audit output | handoff report |
| Paid edition | Constellation Plus |
| Risk requiring review | warning |
